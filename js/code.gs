...
function doGet(e) {
  ...
  try {
    ...
    switch(action) {
      ...
      case 'getConfig':      result = { success: true, config: getAllConfig() }; break;
...
function getConfigVal(key, defaultVal) {
  ...
  let val = String(found.value).trim();

  // FIX GOOGLE SHEETS INDONESIA FORMAT
  // contoh:
  // -6.406.332  -> -6.406332
  // 106.773.285 -> 106.773285
...
function preflightCheck(params) {
  ...
  return {
    ...
    config: {
      office_latitude: getConfigVal('office_latitude', '-6.2088'),
      office_longitude: getConfigVal('office_longitude', '106.8456'),
      max_radius_meters: getConfigVal('max_radius_meters', '50')
    }
  };
}
...
function clockIn(body) {
  ...
  // Validasi radius
  const maxRadius = parseFloat(getConfigVal('max_radius_meters', '50'));
